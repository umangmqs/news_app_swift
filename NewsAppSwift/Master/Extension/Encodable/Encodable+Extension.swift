...
extension Encodable where Self: Decodable {
    ...
    var jsonObject: String {
        guard let data = try? encoded(), let objJSON = try? JSONSerialization.jsonObject(with: data, options: .mutableContainers) else { return "" }
...
extension Decodable {
    init?(dictionary: [String: Any]) {
        do {
            ...
            if let decodedData = decode(Self.self, from: data) {
                self = decodedData
            } else {
                return nil
            }
...
func decode<T>(_: T.Type, from data: Data) -> T? where T: Decodable {
    var decodedData: T?
    do {
